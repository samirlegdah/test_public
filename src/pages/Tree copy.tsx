...
type TreeState = {
  ...
  curNode: {
    ...
    collectedData: [true, true, true, true];
  };
  timestamp: string;
  step: string;
};
...
class SETree extends React.Component<TreeProps, TreeState> {
  ...
  constructor(props: any) {
    ...
    console.log('treeData:', treeDataFromLocalStorage);
  ...
  openModal = (node) => {
    ...
    if (!node.hasOwnProperty('collectedData')) {
      node.collectedData = [true, true, true, true];
    }
  ...
  updatenPhase = (event) => {
    ...
    if (this.state.curNode) {
      node = this.state.curNode;
      node.nPhase = event.target.value;
    }
    this.setState({
      ...this.state,
      nPhase: event.target.value,
      curNode: node,
    });
   ...
   * @param event
   */
  updateCollectedData = (event) => {
    console.log('updateCollectedData:', event.target.value);
    console.log('updateCollectedData:', this.state.curNode);
    let collectedData = this.state.curNode.collectedData;
    console.log('updateCollectedData:', collectedData);
    collectedData[event.target.value] = event.target.checked;
    //graphesToCreate = [1];//{ 1: true, 2: true, 3: true, 4: true, 5: true };

    let node = null;
    if (this.state.curNode) {
      node = this.state.curNode;
      node.collectedData = collectedData;
    }
    this.setState((state) => ({
      ...this.state,
      curNode: node,
  ...
  /**
   * Cerca un nodo in base al title
   *
   * @param title
   * @param treeData
   * @returns false se non trovato / nodo se trovato
   */
  findNodeByTitle = (title, treeData) => {
    ...
    // const rowMeta = rowInfo.node.meta;
    if (data.title.toLowerCase().indexOf(title.toLowerCase()) > -1) {
      return data;
    }
    // data.expanded = !data.disabled;
    console.log('data', data);
    ...
    if (data.children) {
      for (const node of data.children) {
        console.log('node', node);
        if (node.title.toLowerCase().indexOf(title.toLowerCase()) > -1) {
          found = false;
          break;
        }
      }
      if (!found) {
        for (const node of data.children) {
          found = this.findNodeByTitle(title, node);
          if (found) {
            break;
          }
        }
  ...
  addPuntiDiVerifica = (treeData) => {
    ...
    if (data.children) {
      if (data.children.length > 1) {
        var existsVerifica = data.children.find((x) => x.type === 'verifica');
        if (!existsVerifica) {
  ...
  /**
   * Genera elenco dei nodi/dispositivi
   *
   * @returns
   */
  makeButtons() {
  ...
  render() {
    ...
    const canDrop = ({ node, nextParent, prevPath, nextPath }) => {
      ...
      if (node.type && node.type === 'verifica') {
        return false;
      }
    ...
    const powerIconsList = [
      {
        key: 'monofase',
        text: 'monofase',
        value: 'monofase',
        image: { avatar: true, src: 'public/plugins/struttureenergia-inputfeeds-app/img/monofase.png' },
      },
      {
        key: 'trifase',
        text: 'trifase',
        value: 'trifase',
        image: { avatar: true, src: 'public/plugins/struttureenergia-inputfeeds-app/img/trifase.png' },
      },
    ];

    /*
    const treeData = [
      { title: 'Chicken', children: [{ title: 'Egg', expanded: true }], expanded: true },
      { title: 'Fish', children: [{ title: 'fingerline', expanded: true }], expanded: true },
      { title: 'Fish3', children: [{ title: 'asasfingerline', expanded: true }], expanded: true },
    ];
    */
      ...
      <div style={{ height: 800 }}>
        ...
        >
          <div style={{ width: '100%', height: '50px', padding: '10px', backgroundColor: 'white' }}>
            {this.state.step === '1' && (
              <button style={{ float: 'left', width: '100%' }} onClick={(event) => this.saveTree(this.state.treeData)}>
                Salva
              </button>
            )}
            {this.state.step === '2' && (
        ...
        <SortableTree
          ...
          canDrop={canDrop}
          canNodeHaveChildren={(node) => node.type !== 'verifica'}
          rowHeight={70}
          slideRegionSize={100}
            ...
            title: (
              <div>
                ...
                <img
                  ...
                  src={'public/plugins/struttureenergia-inputfeeds-app/img/' + node.icon + '.png'}
                  onClick={(event) => {
                    // this.openModal();
                ...
                <img
                  style={{
                    width: '32px',
                    border: node.nPhase ? '1px solid' : '',
                    padding: '0px',
                    backgroundColor: node.disabled === true ? 'white' : 'white',
                  }}
                  src={'public/plugins/struttureenergia-inputfeeds-app/img/' + node.nPhase + '.png'}
                  title={node.nPhase}
                  onClick={(event) => {
                    console.log(node.nPhase);
                    if (this.state.step === '1' && node.nPhase) {
                      if (node.nPhase === 'monofase') {
                        node.nPhase = 'trifase';
                      } else {
                        node.nPhase = 'monofase';
                      }
                      this.setState({
                        ...this.state,
                        nPhase: node.nPhase,
                  ...
                  <Dropdown.Menu>
                    ...
                    <Dropdown.Item
                      ...
                      onClick={function () {
                        ...
                        let trifase = node.nPhase === 'monofase' ? 'no' : '*';
                  ...
                  <i>
                    <button
                      ...
                      onClick={function () {
                        //this.openModal2(node)
                        let trifase = node.nPhase === 'monofase' ? 'no' : '*';
                        gotoDashboard('TempoReale', 'var-dispositivo=' + node.title + '&var-trifase=' + trifase);
                        // makeDashboard(node.title, 9);
                      }}
                    >
                      D
                ...
                <Menu id={'MENU_ID'}>
                  <Item onClick={() => console.log('yo')}>Item 1</Item>
                  <Item onClick={() => console.log('yo')}>Item 2</Item>
                  <Separator />
                  <Item disabled>Disabled</Item>
                  <Separator />
                  <Submenu label="Foobar">
                    <Item onClick={() => console.log('yo')}>Sub Item 1</Item>
                    <Item onClick={() => console.log('yo')}>Sub Item 2</Item>
                  </Submenu>
                </Menu>
              </div>
            ),
            ...
            <div className="row pb-3">
              <div className="col-sm-12">
                ...
                <div className="row">
                  ...
                  <div className="form-control col-sm-9">{this.state.curNode.type}</div>
                </div>
                {this.state.curNode.type.indexOf('misura') > -1 && (
                  <div className="row">
                    ...
                    <div className="form-control col-sm-9">
                      <Dropdown
                        ...
                        value={this.state.curNode.icon}
                      />
                    </div>
                  </div>
                )}
                {false && this.state.curNode.type.indexOf('misura') > -1 && (
                  <div className="row">
                ...
                <div className="row">
                  ...
                  <select className="form-control col-sm-9" value={this.state.nPhase} onChange={this.updatenPhase}>
                ...
                {false && (
                  <div className="row graphsBlock" id="graphsBlock">
                    <div className="col-sm-3 mt-4">
                      <input
                        type="checkbox"
                        checked={this.state.curNode.collectedData[0]}
                        value="0"
                        onChange={(e) => this.updateCollectedData(e)}
                      />
                    ...
                    <div className="col-sm-3 mt-4">
                      <input
                        type="checkbox"
                        checked={this.state.curNode.collectedData[1]}
                        value="1"
                        onChange={(e) => this.updateCollectedData(e)}
                      />
                    ...
                    <div className="col-sm-3 mt-4">
                      <input
                        type="checkbox"
                        checked={this.state.curNode.collectedData[2]}
                        value="2"
                        onChange={(e) => this.updateCollectedData(e)}
                      />
                    ...
                    <div className="col-sm-3 mt-4">
                      <input
                        type="checkbox"
                        checked={this.state.curNode.collectedData[3]}
                        value="3"
                        onChange={(e) => this.updateCollectedData(e)}
                      />
                ...
                <div className="row">
                  ...
                  <input
                    value={this.state.curNode.destinazione}
                    className="form-control col-sm-9"
