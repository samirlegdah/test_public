...
export function insertNode({
  ...
  if (!treeData && targetDepth === 0) {
    return {
      treeData: [newNode],
      treeIndex: 0,
      path: [GetNodeKeyFunction({ node: newNode, treeIndex: 0 })],
      parentNode: null,
    };
  }
...
export async function getDashboardsByTitle(title) {
  const resp = await fetch('/api/search?query=' + encodeURI(title), {
    headers: {
      //'x-rre-apikey': feedStore.EMONCMS_API_KEY,
      'Content-Type': 'application/json',
    },
...
export function gotoDashboard(name, queryString) {
  fetch('/api/search?query=' + name, {
    headers: {
      //'x-rre-apikey': feedStore.EMONCMS_API_KEY,
      'Content-Type': 'application/json',
    },
    ...
    .then((respJson) => {
      ...
      window.open(
        respJson[0].url + '?' + queryString,
        '_blank' // <- This is what makes it open in a new window.
      );
      //location.href = respJson[0].url + '?' + queryString;
    })
    .catch((err) => {
      console.log(err);
    });
...
export function makeDashboard(node, folderId, graphesToCreate, dashboardId = null, dashboardUid = null) {
  console.log('in makedashboard', node);
  let panelsRealTime = {
    1: {
      datasource: 'OnlyRead',
      description: '',
      fieldConfig: {
        defaults: {
          ...
          decimals: 2,
          displayName: 'Intensità di corrente',
          mappings: [],
          thresholds: {
            mode: 'absolute',
            steps: [
    ...
    4: {
      ...
      fieldConfig: {
        defaults: {
          ...
          decimals: 2,
          displayName: 'Fattore di potenza',
          mappings: [],
          thresholds: {
            mode: 'absolute',
            steps: [
  ...
  let y = 0;
  for (let i = 1; i <= 4; i++) {
    if (graphesToCreate[i]) {
      let ids = null;
      let feeds = null;
      //In base al tipo di grafico, prende i feed relativi e poi...
      switch (i) {
        case 1:
          //Intensità di corrente
          //Prende tutti i campi corrente_x
          feeds = node.meta.feeds.filter((x) => x.name.indexOf('corrente_') === 0);
          break;
        case 2:
          //Tensione
          feeds = node.meta.feeds.filter((x) => x.name.indexOf('volt_') === 0);
          break;
        case 3:
          //Fattore potenza
          feeds = node.meta.feeds.filter((x) => x.name.indexOf('fattore_potenza_') === 0);
          break;
        case 4:
          //Potenza attiva
          feeds = node.meta.feeds.filter((x) => x.name.indexOf('potenza_') === 0);
          break;
      }
      console.log('Feeds', feeds);

      //...sostituisce il marcatore {{params_id}} con l'id del feed
      for (let f of feeds) {
        let t = { ...panelsRealTime[i] }; //copia l'oggetto
        t.id = count++;
        t.title += ' (' + f.name + ')';
        t.targets[0].params.unshift(['id', f.id]); //Appende all'inizio
        t.gridPos = { h: 6, w: 6, x: x, y: y };
        x = x + 6;
        console.log('ADD panel:', x, y, t);
        panels.push(t);
      }
  ...
  const resp = fetch('/api/dashboards/db', {
    headers: {
      //'x-rre-apikey': feedStore.EMONCMS_API_KEY,
      'Content-Type': 'application/json',
    },
    ...
    .then((resp) => {
      if (!resp.ok) {
        alert('Errore nella creazione della dashboard');
...
export async function getDataSourceById(id) {
  const resp = await fetch('/api/datasources/' + encodeURI(id), {
    headers: {
      //'x-rre-apikey': feedStore.EMONCMS_API_KEY,
      'Content-Type': 'application/json',
    },
