...
export const Inputs = ({ plugin }: Props) => {
  ...
  useEffect(() => {
    ...
    feedStore.setKey(plugin.meta.jsonData['dataSourceId']);
    feedStore.addListener(0, function (newFeeds: any[]) {
      setError(false);
      console.log('AppConfigInputs.tsx:', newFeeds);
      setData(newFeeds);
    });
  ...
  function InitProcesses() {
    fetch('/api/datasources/proxy/' + plugin.meta.jsonData['dataSourceId'] + '/precesslist-initial', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
