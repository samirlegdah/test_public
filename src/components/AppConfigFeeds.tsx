...
export const Feeds = ({ plugin }: Props) => {
  ...
  useEffect(() => {
    if (typeof plugin.meta.jsonData['dataSourceId'] === 'undefined') {
      setError(true);
      setErrorMsg('Selezionare una datasource nel pannello di amministrazione');
      return;
    }
    feedStore.setKeyFromTree(plugin.meta.jsonData['dataSourceId']);
  ...
  function flatTree(datain, dataout) {
    for (const node of datain) {
      // console.log(node)
      dataout.res.push(node);

      if (node.children.length > 0) {
        flatTree(node.children, dataout);
      }
    }
    return dataout;
  }
  ...
  function handleInputs() {
    ...
    var dataout = { res: [] };
    flatTree(data, dataout);
    var inps = dataout.res.map((node, index) => (
      <NodeFeed
        key={index}
        name={node.title}
            ...
            <div className="col-sm-6">
              <button
                ...
                onClick={() => {
                  setUpdating(true);
                  let csvToLoad = csv;
                  const dataToFeed = `[[${csvToLoad.join('],[')}]]`;
                  //console.log("dataToFeed", dataToFeed)
